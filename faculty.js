...
(function harden() {
  ...
  shield.innerHTML = `
    <div>
      <h2 style="font-size:24px;margin:0 0 8px">Hold up.</h2>
      <p style="opacity:.85;white-space:pre-line">${SHIELD_MSG}</p>
    </div>`;
  document.documentElement.appendChild(shield);

  // --- Context menu & selection blockers ---
  window.addEventListener('contextmenu', e => e.preventDefault(), {capture:true});
  ['copy','cut','paste'].forEach(type => {
    document.addEventListener(type, (e) => {
      // Nuke clipboard on copy/cut; block paste
      try {
        if (type !== 'paste' && (e.clipboardData || window.clipboardData)) {
          (e.clipboardData || window.clipboardData).setData('text/plain', '');
        }
      } catch(_) {}
      e.preventDefault();
    }, true);
  });
  // Fallback selection kill
  ...
  const blockedCombos = new Set([
    ...
    'CTRL+INSERT',         // Copy (alt)
    'SHIFT+INSERT',        // Paste (alt)
    'META+ALT+I',          // DevTools mac
    'META+ALT+J',
    'META+ALT+C',
    'META+P', 'META+S', 'META+U', 'META+C', 'META+X'
  ]);
  document.addEventListener('keydown', (e) => {
    const key = e.key.toUpperCase();
    const ctrl = (isMac ? e.metaKey : e.ctrlKey);
    const combo = [
      ctrl ? (isMac ? 'META' : 'CTRL') : '',
      e.shiftKey ? 'SHIFT' : '',
      e.altKey ? 'ALT' : '',
      key
    ].filter(Boolean).join('+');
    ...
    if (blockedCombos.has(combo) || blockedCombos.has(key)) {
      e.preventDefault();
      e.stopPropagation();
      // Subtle deterrent
      if (/CTRL\+P|META\+P|F12|I|J|C/.test(combo)) showShield(true);
    }
  }, true);

  // --- Print deterrent (before print dialog) ---
  ['beforeprint', 'afterprint'].forEach(evt => {
    window.addEventListener(evt, () => {
      if (evt === 'beforeprint') showShield(true);
      if (evt === 'afterprint') showShield(false);
    });
  });
  ...
  function showShield(show) {
    shield.classList.toggle('visible', !!show);
    // Optionally blur underlying body to annoy inspectors
    document.documentElement.style.filter = show ? 'blur(3px)' : '';
  }

  // --- Extra: disable iframe embedding & tighten caching (needs headers too) ---
  // In JS we can at least remove if framed (clickjacking / simple leeching)
  try {
  ...
  // Clean up on unload
  window.addEventListener('unload', () => clearInterval(devtoolsInterval));
})();
